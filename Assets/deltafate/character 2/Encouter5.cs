...
public class Encouter5 : MonoBehaviour
{
    ...
    public void newmetod()
    {
        ...
        for (int i = 0; i < effect.ideffect.Count; i++)
        {
            ...
            for (int i2 = 0; i2 < GameObject.FindObjectsOfType<Encouter5>().Length; i2++)
            {
                if (effect.ideffect[i] == "damage")
                {
                    GameObject.FindObjectsOfType<Encouter5>()[i2].hp -= effect.power[i];
    ...
    void Update()
    {
        ...
        if (player2)
        {
            ...
            if (player2.GetComponent<igrok1>().sheld <= 0)
            {
                ...
                if (PlayerPrefs.GetInt("command", 0) == 4)
                {
                    ...
                    s = JsonUtility.FromJson<battlesave>(File.ReadAllText(@"DELTAFATE/siji"+nameplus+"/bposition.un"));
        ...
        if (PlayerPrefs.GetInt("command", 0) == 1)
        {
            ...
            if (hp <= 0 || mp <= 0)
            {
                ...
                PlayerPrefs.SetFloat("attack", 0);
                if (File.Exists(@"DELTAFATE/siji" + nameplus + "/bposition.un"))
                {
            ...
            if (hp <= 0 || mp <= 0)
            {
                if (Type == 1)
                {
                    ...
                    if (PlayerPrefs.GetInt("r1", 0) == 1)
                    {
                        ...
                        PlayerPrefs.SetFloat("attack", 0);
                        if (File.Exists(@"DELTAFATE/siji" + nameplus + "/inventory.un"))
                        {
                            s2 = JsonUtility.FromJson<items>(File.ReadAllText(@"DELTAFATE/siji" + nameplus + "/inventory.un"));
                            s2.itemid.Add(3);
                            
                            File.WriteAllText(@"DELTAFATE/siji" + nameplus + "/inventory.un", JsonUtility.ToJson(s2));
                        ...
                        if (File.Exists(@"DELTAFATE/siji" + nameplus + "/bposition.un"))
                        {
                            ...
                            s.level += 224;
                            File.WriteAllText(@"DELTAFATE/siji" + nameplus + "/bposition.un", JsonUtility.ToJson(s));
                            SceneManager.LoadScene(s.idscene[0]);
                        }
                    }
                }
