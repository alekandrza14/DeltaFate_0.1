Shader "UnityLibrary/2D/Special/RayMarching"
{
	SubShader
	{
		...
		Pass
		{
			...
			{
			  float4 sphere = float4(0, 2, 6, 1); // x,y,z,radius
			  float sphereDist = length(p - sphere.xyz) - sphere.w;
			  
			  float distance = min(sphereDist, sphereDist); // return closest distance
			  return distance;
			}
			...
			{
			  float2 uv = (i.uv - 0.5);
			  float3 col = float3(2,0,0);

			  float3 rayOrigin = float3(0,3,0);
